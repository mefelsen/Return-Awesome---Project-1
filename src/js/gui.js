...
function oneShipmode() {
  //console.log("in here!")
  let coord = exec.admir1.findShipByCoord(exec.admir2.aIcoord)

  var arr = exec.admir1.fleet[coord].getCoords()
  var shipsize = arr.length
  console.log(arr)
  console.log(shipsize)

  var guessArr = exec.admir2.guessedarr
  var guessarrSize = guessArr.length
  console.log(guessArr)
  console.log("for the guess " + guessarrSize)
  //console.log(guessArr+" for guessed")
  for (let i = 0; i < shipsize; i++) {
    let checker = false
    for (let j = 0; j < guessarrSize; j++) {
      if (arr[i] == guessArr[j]) {
        checker = true
      }
    }
    if (!checker) {
      exec.admir2.guessedarr.push(arr[i])
      buttonHandler("fire1", arr[i])
      exec.admir2.aIcoord = arr[i]
      break
    }
  }
  if (shipsize == guessarrSize) {
    console.log("switch off oneshipmode")
    for (let i = 0; i < guessarrSize; i++) {
      exec.admir2.guessedarr.pop()
    }
    for (let i = 0; i < shipsize; i++) {
      arr.pop()
    }
